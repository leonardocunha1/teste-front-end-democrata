  ...
  return (
    <section className="bg-stone-100 px-4 py-6">
      <div className="flex flex-col items-center">
        ...
        <div className="mb-6 h-1 w-20 bg-stone-900"></div>
        <ul className="grid gap-4 sm:grid-cols-2 sm:gap-20 lg:grid-cols-3">
          {visibleProducts.map((product) => (
            <NewArrivalsItem key={product.id} product={product} />
          ))}
        </ul>
        <button
          onClick={toggleShowMore}
          className="mt-6 rounded-full border-2 border-stone-900 px-6 py-2"
        >
